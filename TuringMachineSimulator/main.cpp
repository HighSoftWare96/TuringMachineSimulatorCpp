...
/// Funzione per mostrare a video un percorso separato per il cambio del nastro della MdT.
...
int main() {
	...
	while (choice != 4) {
		...
		/// Input scorretti (non nel range e non numeri) generano un'eccezione.
		try {
		...
		case 1:
			...
			break;
		case 2:
			/// Se non è ancora stata creata una delle due allora avviso l'utente che non può eseguire la MdT. 
...
void machineMenu(TuringMachine *machine, TuringMachineState *tape) {
	...
	/// Continuo a chiedere finchè non c'è almeno un elemento negli stati o inserisce stati > 0.
	while (intInput >= 0 || machine->getStates().size() == 0) {
		...
		/// Aggiungo un nuovo stato alla Mdt. Se ritorna false lo stato non era ammesso.
	...
	/// simboli e interi da richiedere e salvarsi per inserire una transizione nella MdT.
	int stateToInsert;
	char symbolToInsert;
	int stateNToInsert;
	...
	do {
		...
		/// Restituisco una rappresentazione a schermo della transizione appena creata e inserita.
