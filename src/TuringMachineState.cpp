...
void mdtModels::TuringMachineState::execute() {
  ...
  while (mdtContinues) {
    ...
    currentTransition = getTransition(currentState.value, (*machineTape)[position]);
    ...
    if (mdtContinues) {
      ...
      currentState.value = currentTransition->state;
  ...
  cout << "Stato n." << to_string(currentState.value) << ", "
       << (currentState.value == machine->getFinalState().value ? " " : " non ")
       << "finale";
}
...
mdtModels::TuringMachineState::getTransition(int currentState,
  ...
  /// itero per tutta la mappa tramite il puntatore/iterator alla mappa stessa,
  /// se trovo la transizione restituisco il suo puntatore
  for (std::map<TransitionBase, TransitionValue>::iterator it =
           transitions->begin();
       it != transitions->end(); it++) {
    /// trovo una transizione con l'asterisco e quello stato oppure quello stato
    /// e quel simbolo
...
std::string mdtModels::TuringMachineState::getMachineStatus(
    TransitionValue const *currentTransition) {
  ...
  result.append("\nSTATO CORRENTE: ");
  result += to_string(currentState.value);
  ...
  result += (currentTransition->nextMove == L
                 ? "L"
                 : ((currentTransition->nextMove == R) ? "R" : "N"));
  result.append("\nNASTRO: ");
  result += printTape();
  result += "\n";
  return result;
